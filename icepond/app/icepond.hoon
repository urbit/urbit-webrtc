...
++  on-watch
  ...
  ?+  watch-path  (on-watch:default watch-path)
      [%ice-servers * ~]
      =/  rqid  +<.watch-path
      ?:  (~(has by serving-requests.state) rqid)  ~|("Already got a request with id: {<rqid>}" !!)
      ...
      :_  this(serving-requests.state (~(put by serving-requests.state) rqid wires-set), watching-wires.state (~(uni by watching-wires.state) wires-map))
...
++  on-agent  :: TODO: handle failed poke-acks from spider
    ...
    ?+  wire  (on-agent:default wire sign)
      ...
      ?~  watching-wire
        ~&  >>>  "icepond: got on-agent on no-longer-watching wire {<wire>}"  `this
      ?+  -.sign  `this
        %watch-ack
        ?~  p.sign
           ~&  >  "icepond: watch-ack {<wire>}"  `this
        ...
        ::
        %kick  ~&  >  "icepond: kicked from {<path.u.watching-wire>} on {<ship.u.watching-wire>}"
        ...
        ?-  fetch-kind.u.watching-wire
          ...
          ::
          %watch-ted
          ?+  p.cage.sign  ~|  "Unexpected mark from thread result: {<p.cage.sign>}"  !!
            %thread-fail
